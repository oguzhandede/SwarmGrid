...
public class ZoneRepository : IZoneRepository
{
    ...
    public async Task<IEnumerable<Zone>> GetAllAsync(string tenantId)
    {
        return await _context
            .Zones.Where(z => z.TenantId == tenantId)
            .OrderBy(z => z.SiteId)
            .ThenBy(z => z.Name)
            .ToListAsync();
    }
    ...
    {
        return await _context
            .Zones.Where(z => z.TenantId == tenantId && z.SiteId == siteId)
            .OrderBy(z => z.Name)
            .ToListAsync();
    }
    ...
    {
        // Load all zones and filter in memory (CameraIds.Contains cannot be translated to SQL)
        var zones = await _context.Zones.ToListAsync();
        return zones
            .Where(z =>
                z.CameraId == cameraId || (z.CameraIds != null && z.CameraIds.Contains(cameraId))
            )
            .OrderBy(z => z.Name)
            .ToList();
    }
    ...
    {
        return await _context.Zones.FirstOrDefaultAsync(z =>
            z.TenantId == tenantId && z.SiteId == siteId && z.ZoneId == zoneId
        );
    }
