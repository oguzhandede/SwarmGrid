...
public class RiskController : ControllerBase
{
    ...
    public RiskController(IRiskEventRepository riskEventRepository, ILogger<RiskController> logger)
    {
    ...
    public async Task<ActionResult<RiskEventDto>> GetCurrentRisk(string zoneId)
    {
        ...
        return Ok(
            new RiskEventDto(
                ...
                0.0, // Density - would need to be stored with risk event
                0 // PersonCount - would need to be stored with risk event
            )
        );
    }
    ...
    )
    {
        ...
        _logger.LogInformation("Event {EventId} acknowledged by {User}", eventId, request.UserId);
